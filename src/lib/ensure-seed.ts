...
export async function ensureSeed() {
  if (seeded) return;
  if (seeding) {
    while (seeding) await new Promise((r) => setTimeout(r, 100));
    return;
  }
  seeding = true;
  try {
    ...
    await prisma.crop.createMany({
      data: [
        { name: 'Tomato', fieldId: field1.id, variety: 'Roma', stage: 'fruiting', status: 'healthy' },
        { name: 'Potato', fieldId: field2.id, variety: 'Kufri', stage: 'vegetative', status: 'healthy' },
        { name: 'Cotton', fieldId: field3.id, variety: 'Bt Cotton', stage: 'flowering', status: 'healthy' },
        { name: 'Rice', fieldId: field4.id, variety: 'Basmati', stage: 'seedling', status: 'healthy' },
      ],
    });
